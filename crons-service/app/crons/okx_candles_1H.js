...
const logger = require('log4js').getLogger('okx-candles-1H-cron');

async function okxCandles1H() {
  try {
    const coins = await CoinsList.find({});
    const dmCheck = await HourlyModel.find({});
    if (dmCheck.length === 0) {
      for (let i = 0; i < coins.length; i++) {
        ...
        await HourlyModel.insertMany(array);
        logger.info(
          `1H historical data added for ${coins[i].base_asset}-${coins[i].quote_asset}`
        );
      }
    } else {
      for (let i = 0; i < coins.length; i++) {
        const latestTs = await HourlyModel.findOne({
